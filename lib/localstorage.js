		...
		fs.readFile(file.path, function (err, data) {
			if(err) {
				...
				if(context.encryptionEnabled) {
					data = cryptojs.AES.encrypt(data, context.encryptionKey);
				}
					...
					if(err) {
						...
						if(bundle.files) {
							...
							_.each(bundle.files, function(file) {
								if(context.encryptionEnabled) {
									fs.readFile(path.join(basedir, file.id + '.binary'), function (err, data) {
										data = cryptojs.AES.decrypt(data, context.encryptionKey);
										archive.file(data, { name: file.name });
										completed();
									});
								} else {
