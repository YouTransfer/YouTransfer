...
LocalFileStorage.prototype = new EventEmitter;
				...
				fs.readFile(path.join(basedir, file), 'utf-8', function(err, data) {
					...
					if(context.expires) {
						...
						if(Date.compare(expires, new Date()) < 0) {
							del([context.path, context.jsonPath], function(err, files) {
								files = files.length > 0 ? files : [context.path, context.jsonPath];
								self.emit('localstorage.purged', err, files);
							});
						}
					}
				});
