...
function LocalFileStorage(options) {
	...
	if(_.isObject(this.options)) {
		...
		this.encryptionIv = security.encryptionIv;
...
LocalFileStorage.prototype.upload = function(file, context, next) {
	...
	fs.mkdir(basedir, function() {
		...
		if(self.encryptionEnabled && self.encryptionKey && self.encryptionIv) {
			var cipher = crypto.createCipheriv(algorithm, self.encryptionKey,self.encryptionIv);
							...
							_.each(bundle.files, function(file) {
								...
								if(self.encryptionEnabled && self.encryptionKey && self.encryptionIv) {
									fs.readFile(filePath, function(err, data) {
										var decipher = crypto.createDecipheriv(algorithm, self.encryptionKey, self.encryptionIv);
										var buffer = Buffer.concat([decipher.update(data) , decipher.final()]);
										archive.append(buffer.toString(), { name: file.name });
										completed();
									});
								} else {
		...
		if(token) {
			...
			fs.readFile(tokenPath, 'utf8', function(err, data) {
				try {
					...
					if(self.encryptionEnabled && self.encryptionKey && self.encryptionIv) {
						var decipher = crypto.createDecipheriv('aes-256-ctr', self.encryptionKey, self.encryptionIv);
