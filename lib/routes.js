...
Router.prototype.settingsFinalise = function() {
	return function(req, res, next) {
		...
		var settings = _.merge(req.params.settings, {
			state: {
				finalised: true
			}
		});
...
Router.prototype.settingsUnlock = function() {
	return function(req, res, next) {
		...
		youtransfer.settings.get(function(err, settings) {
			req.errors.parse(err);
			try {
				if(settings.state.unlockCode === req.params.unlockCode) {
...
Router.prototype.login = function() {
	return function (req, res) {
		res.setCookie('token', req.user, {
			path: '/',
			expires: new Date().add({ minutes: 20 }),
			secure: (req.socket.encrypted ? true : false),
			httpOnly: true,
			maxAge: 1200
		});
