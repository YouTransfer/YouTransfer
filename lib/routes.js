...
Router.prototype.login = function(passport) {
	return function(req, res, next) {
		//req.errors.register(new Error('An error occured while logging in'));
		youtransfer.settings.get(function(err, settings) {
			//passport.authenticate('local', {})(req, res, next);
			passport.authenticate('local', {
				successRedirect: settings.general.baseUrl + '/',
				failureRedirect: settings.general.baseUrl + '/',
				failureFlash: true
			})(req, res, next);
		});
	};
...
Router.prototype.baseRedirect = function(url) {
	return function(req, res) {
		youtransfer.settings.get(function(err, settings) {
			res.redirect(settings.general.baseUrl + url);
		});
	};
};
...
Router.prototype.settingsRedirect = function() {
	return function(req, res) {
		youtransfer.settings.get(function(err, settings) {
			res.redirect(settings.general.baseUrl + '/settings/general');
		});
	};
};
	...
	return function(req, res, next) {
		...
		youtransfer.settings.finalise(req.params.settings.state.unlockCode, function(err) {
			...
			if(req.errors.exist()) {
				...
				youtransfer.settings.get(function(err, settings) {
					res.redirect(settings.general.baseUrl + '/');
					next();
				});
			}
		});
