...
Router.prototype.settingsSaveByName = function() {
	...
	return function(req, res, next) {
		...
		} else if(req.params.name == 'finalise') {
			self.settingsFinalise()(req, res, next);
		} else {
...
Router.prototype.accounts = {
	...
	byName: function() {
		return function(req, res, next) {
			var page = req.params.name || 'general';
			req.errors.register(new Error('An unknown error occurred while retrieving the account page'));

			var context = {};
			if(page === 'users') {
				context.users = youtransfer.users.export('allowed');
			} else if(page === 'approval') {
				context.users = youtransfer.users.export('pending');
			} else if(page === 'invitations') {
				context.users === youtransfer.users.export('invitations');
			}

			res.process('accounts.' + page + '.html', context, next);
		};
	},
	...
	approve: function() {
		return function(req, res, next) {
			try {
				...
				var user = youtransfer.users.get(id, 'pending');
				if(user) {
					user.status = 'allowed';
					if(youtransfer.users.set(user.id, user, user.status)) {
						if(youtransfer.users.del(user.id, 'pending')) {
							res.redirect('/accounts/approval');
						} else {
					...
					throw new Error('Incorrect user provided');
				}
			} catch(err) {
				req.errors.parse(err);
				res.process('accounts.approval.html', null, next);
			}
		}
	}
};

Router.prototype.providers = {
	...
	push: function() {
		...
		return function(req, res, next) {
			...
			youtransfer.settings.push(value, function(err) {
				req.errors.parse(err);
				res.process('accounts.providers.html');
			});
		};
	}
};
...
Router.prototype.authenticate = function() {
	return function(req, res) {
		var strategy = passport.strategies[req.params.provider];
		if(strategy) {
			passport.authenticate(strategy.name, { successRedirect: '/' })(req, res, function(err) {
				res.process('404.html');
			});
		} else {
			res.process('404.html');
		}
