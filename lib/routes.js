...
Router.prototype.settingsFinalise = function() {
	return function(req, res, next) {
		...
		var code = req.params.settings.state.unlockCode;
		youtransfer.settings.finalise(code, function(err) {
			req.errors.parse(err);
			if(req.errors.exist()) {
...
Router.prototype.settingsUnlock = function() {
	return function(req, res, next) {
		...
		var code = req.params.unlockCode;
		youtransfer.settings.unlock(code, function(err) {
			req.errors.parse(err);
			if(req.errors.exist()) {
