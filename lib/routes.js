'use strict';

var fs = require("fs");
var mime = require('mime');
var _ = require("lodash");
var md5 = require("md5");
var filesize = require("filesize");

//TODO: the actual interpretation of the request should be placed
//in a separate module, instead of inline in the router module

module.exports = function(app, nconf) {


	app.post(/^\/upload/, function(req, res, next) {

		var isXmlHtppRequest = req.files['dz-payload'];
		var file = req.files.payload || req.files['dz-payload'];
		var fileId = md5(file.name + (Math.random() * 1000));
		var fileMetaData = {
			id: fileId,
			name: file.name,
			size: file.size,
			type: file.type,
			lastModifiedDate: file.lastModifiedDate
		};

		var destFile = './uploads/' + fileId + '.binary';
		var destFileJson = './uploads/' + fileId + '.json';

		var response = _.assign({ 
			success: false,
			isPostback: true, 
			isXmlHtppRequest: _.isNull(isXmlHtppRequest) ? false : true,
			link: nconf.get('baseUrl') + '/download/' + fileId + '/',
			filesize: filesize(file.size)
		}, fileMetaData);

		fs.mkdir('./uploads', function() {
			fs.readFile(file.path, function (err, data) {
				response.success = _.isNull(err);
				if(err) {
					if(isXmlHtppRequest) {
						res.json(response);
					} else {
						res.render("index.html", response);
					}
				} else {
					fs.writeFile(destFile, data, function(err) {
						response.success = _.isNull(err);
						if(err) {
							if(isXmlHtppRequest) {
								res.json(response);
							} else {
								res.render("index.html", response);
							}
						} else {
							fs.writeFile(destFileJson, JSON.stringify(fileMetaData), 'utf-8', function(err) {
								response.success = _.isNull(err);
								if(isXmlHtppRequest) {
									res.json(response);
								} else {
									res.render("index.html", response);
								}
							});
						}
					});
				}
			});
		});
	});

	app.get(/^\/download\/(.*)\//, function(req, res, next) {
		var token = req.params[0];

		//TODO: this is duplicate code.
		try {
			var metadata = require('../uploads/' + token + '.json');
			if(metadata) {

				var file = __dirname + '/../uploads/' + token + '.binary';
				var mimetype = mime.lookup(file) || metadata.type;

				res.setHeader('Content-disposition', 'attachment; filename="' + metadata.name + '"');
				res.setHeader('Content-length', metadata.size);
				res.setHeader('Content-type', mimetype);

				var filestream = fs.createReadStream(file);
				filestream.pipe(res);
			}
		} catch (exp) { 
			res.redirect("/");
		}		
	});

	app.post(/^\/download/, function(req, res, next) {
		var token = req.params.token;

		//TODO: this is duplicate code.
		try {
			var metadata = require('../uploads/' + token + '.json');
			if(metadata) {

				var file = __dirname + '/../uploads/' + token + '.binary';
				var mimetype = mime.lookup(file) || metadata.type;

				res.setHeader('Content-disposition', 'attachment; filename="' + metadata.name + '"');
				res.setHeader('Content-length', metadata.size);
				res.setHeader('Content-type', mimetype);

				var filestream = fs.createReadStream(file);
				filestream.pipe(res);
			}
		} catch (exp) { 
			res.redirect("/");
		}
	});

	app.post(/^\/settings/, function(req, res, next) {
		var context = req.params["settings"];
		var settings = JSON.parse(fs.readFileSync('./settings.json', 'utf8')) || {};
		_.assign(settings, _.pick(context, _.identity));

		fs.writeFile('./settings.json', JSON.stringify(settings), function(err) {
			res.setHeader('Cache-Control', 'private, max-age=0, proxy-revalidate, no-store, no-cache, must-revalidate');
			res.render("settings.html", {
				isPostback: true,
				success: _.isNull(err) 
			});
		});
	});

	// ------------------------------------------------------------------------------------------ Static Server routes

	// Service static resources
	app.get(/^(\/v\d*)?\/(js|css|assets|fonts|img|sounds)\/(.*)/, function(req, res, next) {
		app.static.serveFile('/' + req.params[1] + '/' + req.params[2], 200, { server: 'youtransfer.io', 'Cache-Control': 'max-age=' + nconf.get('CACHE_MAX_AGE') }, req, res);
	});

	// ------------------------------------------------------------------------------------------ Redirect all other routes to client-side

	// Try to render page based on path
	app.get(/^\/(.*)/, function(req, res, next) {
		var page = req.params[0] || 'index';
		res.setHeader('Cache-Control', 'private, max-age=0, proxy-revalidate, no-store, no-cache, must-revalidate');
		res.render(page + ".html");
	});

}