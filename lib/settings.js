...
Settings.prototype.push = function(settings, next) {
	...
	fs.readFile(self.options.path, self.options.encoding, function(err, content) {
		try {
			...
			// Combine to pick up the possibly modified salt
			var output = _.defaultsDeep(_.cloneDeep(settings), defaults);
			var security = output.security || {};
			var salt = nconf.get('ENCRYPTIONKEY') || security.encryptionKey;
			// Only hash new passwords. Those in 'defaults' have alreaby been hashed.
			settings = self._hashPasswords(settings, salt);

			output = _.defaultsDeep(settings, defaults);
